...
function buildFilterValue(context: {
  id?: string;
  definitionId?: string;
  key: string;
  itemId: string;
...
function buildSearchValue(context: {
  id?: string;
  definitionId?: string;
  key: string;
  itemId: string;
...
function buildSortValue(context: {
  id?: string;
  definitionId?: string;
  key: string;
  itemId: string;
...
async function getDefinitionByUserKey(ctx: any, userToken: string, key: string) {
  return await ctx.db
    .query('user_list_definitions')
    .withIndex('by_user_key', (q: any) => q.eq('userToken', userToken).eq('key', key))
    .unique();
}
...
async function applyUserListCountDelta(
  ...
  for (const countItemId of countItemIds) {
    if (accessScope === 'PUBLIC') {
      await adjustUserListPublicCount(ctx, key, filterValue, countItemId, delta);
    }
    ...
    if (typeof privacy === 'object' && privacy !== null) {
      for (const allowedUserId of privacy.allowList) {
        await adjustUserListSharedCount(ctx, allowedUserId, key, filterValue, countItemId, delta);
      }
    }
  }
}
  ...
  handler: async (ctx, args) => {
    ...
    const item = await ctx.db
      ...
      .withIndex('by_user_key_item', (q) =>
        q.eq('userToken', userToken).eq('key', args.key).eq('itemId', args.itemId)
      )
      .unique();
  ...
  handler: async (ctx, args) => {
    ...
    const publicCount = await ctx.db
      ...
      .withIndex('by_key_filter_item', (q) =>
        q.eq('key', args.key).eq('filterValue', args.filterFor).eq('itemId', countItemId)
      )
      .unique();
...
export async function setUserListForToken(
  ctx: MutationCtx,
  userToken: string,
  args: SetUserListArgs
) {
  const now = Date.now();
  ...
  const previousItems =
    existingDefinition && definitionConfigChanged
      ? await ctx.db
          .query('user_lists')
          .withIndex('by_user_key_sort', (q) => q.eq('userToken', userToken).eq('key', args.key))
          .collect()
      : null;
  ...
  if (!definitionId) throw new Error('Failed to resolve list definition id');
  const finalDefinition = (await ctx.db.get(definitionId)) as Doc<'user_list_definitions'> | null;
  if (!finalDefinition) throw new Error('Failed to load final list definition');
  await syncListPermissions(ctx, definitionId, userToken, args.key, finalDefinition.privacy);
  ...
  const filterValue = buildFilterValue(baseContext);
  const searchValue = buildSearchValue({ ...baseContext, filterValue });
  const sortValue = buildSortValue({ ...baseContext, filterValue, searchValue });
  const accessScope = privacyToAccessScope(finalDefinition.privacy);
  let listItemId = existingItem?._id;
  ...
  if (existingItem) {
    ...
    listItemId = await ctx.db.insert('user_lists', {
      definitionId,
      userToken,
      key: args.key,
      itemId: args.itemId,
  ...
  if (!listItemId) throw new Error('Failed to resolve list item id');
  ...
  const finalItem = await ctx.db.get(listItemId);
  if (finalItem && (!existingDefinition || !definitionConfigChanged)) {
    const didCountChange =
      !previousSnapshot ||
      previousSnapshot.filterValue !== finalItem.filterValue ||
      !samePrivacy(previousSnapshot.privacy, finalDefinition.privacy as Privacy);
    if (didCountChange) {
      if (previousSnapshot) {
        await applyUserListCountDelta(ctx, {
          ownerUserToken: userToken,
  ...
  return shapeListRecord(finalItem, finalDefinition);
}
...
export const set = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const userToken = identity.subject;
    return await setUserListForToken(ctx, userToken, args);
  },
});
...
export const updatePrivacy = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await syncListPermissions(ctx, definition._id, userToken, args.key, nextPrivacy);
