...
async function adjustUserVarPublicCount(
  ...
  const existing = await ctx.db
    ...
    .withIndex('by_key_filter', (q: any) => q.eq('key', key).eq('filterValue', filterValue))
    .unique();
...
async function adjustUserVarSharedCount(
  ...
  const existing = await ctx.db
    ...
    .withIndex('by_user_key_filter', (q: any) =>
      q.eq('allowedUserId', allowedUserId).eq('key', key).eq('filterValue', filterValue)
    )
    .unique();
...
async function applyUserVarCountDelta(
  ...
  await adjustUserVarOwnerCount(ctx, ownerUserToken, key, filterValue, accessScope, delta);

  if (typeof privacy === 'object' && privacy !== null) {
    for (const allowedUserId of privacy.allowList) {
      await adjustUserVarSharedCount(ctx, allowedUserId, key, filterValue, delta);
    }
  }
}

async function syncPermissions(ctx: any, varId: any, privacy: Privacy) {
  const existing = await ctx.db
    .query('permissions')
...
export const length = query({
  ...
  handler: async (ctx, args) => {
    ...
    const publicCount = await ctx.db
      .query('user_var_public_counts')
      .withIndex('by_key_filter', (q) => q.eq('key', args.key).eq('filterValue', args.filterFor))
      .unique();
    ...
    const sharedCount = await ctx.db
      ...
      .withIndex('by_user_key_filter', (q) =>
        q.eq('allowedUserId', viewerUserId).eq('key', args.key).eq('filterValue', args.filterFor)
      )
      .unique();
...
export async function setUserVarForToken(
  ctx: MutationCtx,
  userToken: string,
  args: SetUserVarArgs
) {
  ...
  if (!finalRecord) throw new Error('Failed to load final variable');
  const nextSnapshot = {
...
export const set = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const userToken = identity.subject;
    return await setUserVarForToken(ctx, userToken, args);
  },
});
