...
export default defineSchema({
  ...
  user_vars: defineTable({
    ...
    .index('by_key_privacy_filter_sort', ['key', 'privacy', 'filterValue', 'sortValue'])
  ...
  user_var_owner_counts: defineTable({
    ...
    key: v.string(),
    filterValue: v.optional(primitiveIndexValue),
    accessScope: v.union(v.literal('PUBLIC'), v.literal('PRIVATE'), v.literal('SHARED')),
    count: v.number(),
  }).index('by_owner_key_filter_scope', ['ownerUserToken', 'key', 'filterValue', 'accessScope']),
  ...
  user_list_shared_counts: defineTable({
    ...
    itemId: v.optional(v.string()),
    count: v.number(),
  }).index('by_user_key_filter_item', ['allowedUserId', 'key', 'filterValue', 'itemId']),

  scheduled_update_targets: defineTable({
    ownerUserToken: v.string(),
    targetType: v.union(v.literal('variable'), v.literal('list')),
    key: v.string(),
    itemId: v.optional(v.string()),
    encodedValue: v.any(),
    batchId: v.string(),
    ...
    .index('by_ownerUserToken_and_targetType_and_key_and_itemId', [
      'ownerUserToken',
      'targetType',
      'key',
      'itemId',
    ])
    .index('by_ownerUserToken_and_batchId', ['ownerUserToken', 'batchId']),

  scheduled_update_batches: defineTable({
    ownerUserToken: v.string(),
    batchId: v.string(),
    state: v.union(v.literal('staged'), v.literal('scheduled')),
    scheduledTime: v.optional(v.number()),
