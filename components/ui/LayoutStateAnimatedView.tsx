...
interface LayoutStateAnimatedViewContainerProps<
  TState extends TransitionStateKey,
...
interface LayoutStateAnimatedViewOptionProps<
  TState extends TransitionStateKey,
> extends PropsWithChildren {
  stateValue: TState;
  page?: number;
...
const webPerformanceStyle =
  Platform.OS === 'web' ? ({ willChange: 'transform, opacity' } as any) : undefined;
...
const getDuration = (animation: LayoutStateAnimatedViewAnimation) =>
...
const LayoutStateAnimatedViewOption = <TState extends TransitionStateKey>(
  _props: LayoutStateAnimatedViewOptionProps<TState>
) => null;

const LayoutStateAnimatedViewOptionContainer = (
  _props: LayoutStateAnimatedViewOptionContainerProps
) => null;
...
const LayoutStateAnimatedViewContainer = <TState extends TransitionStateKey>({
  ...
  const [displayedOption, setDisplayedOption] = useState<ResolvedOption<TState> | undefined>(
    currentOption
  );
  ...
  useEffect(() => {
    ...
    const activeTransition = getTransitionForPageChange(
      previousOption,
      currentOption,
      !highPerformance
    );
    ...
    <View className={className} style={styles.container}>
      <Animated.View
        style={[
          ...
          {
            pointerEvents:
              phase === 'exiting' || phase === 'waiting' || phase === 'swapping' ? 'none' : 'auto',
          },
        ]}>
...
export { createPushInAnimation, fromBottom, fromLeft, fromRight, fromTop };
