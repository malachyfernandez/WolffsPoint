...
## Lazy tab mounting (user disliked all-tabs-fetch-upfront)

- `OperatorGamePage`, `PlayerGamePage`, `NewserGamePage` now track `mountedTabs: ReadonlySet<Tab>` — a tab body only mounts on first visit (`handleTabPress` adds it), then stays mounted + `display:none`'d as before.
- Why keep-mounted-after-visit is required: `useMinimizeTarget.onRestore` calls `onOpenChange(true)` — a setState owned by the tab component. If the tab unmounted, restoring a minimized dialog would silently no-op and the minimized card would vanish. Minimize across tabs preserved.
- Side benefits: first visit to a tab now shows its own LoadingContainer fade (visible per-tab loading UX), and tabs never mount under `display:none`, so the `onLayout`-under-hidden fragility (NewspaperZoomableView etc.) is gone for first visits.
- Behavioral note: the initial "choose your days" setup dialog (`ComprehensiveDaySelector` `showInitialSetupDialog`, inside NightlyPageOPERATOR) now fires on first Nightly visit instead of on game load. `NewspaperPageNEWSER`'s `accepted` record write likewise fires on first newspaper-tab visit.
