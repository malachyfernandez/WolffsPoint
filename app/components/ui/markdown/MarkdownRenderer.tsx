...
export const useMarkdownRendererInputData = () => useContext(MarkdownRendererInputDataContext);
...
export const MarkdownRendererInputDataProvider = ({
  ...
  const contextValue = useMemo(
    () => ({ playerOptions, roleOptions, scriptSources }),
    [playerOptions, roleOptions, scriptSources]
  );
...
const InlineMarkdownWithInputs = ({
  ...
  return (
    <View
      className={`w-full flex-row flex-wrap items-center gap-2 ${textAlign === 'center' ? 'justify-center' : 'justify-start'}`.trim()}>
      {segments.map((segment, index) => {
        ...
        if (segment.type === 'link') {
          const normalizedUrl = segment.url.match(/^https?:\/\//)
            ? segment.url
            : `https://${segment.url}`;
          return (
            <Pressable
        ...
        const segmentWeight =
          segment.type === 'bold' ? 'bold' : segment.type === 'code' ? 'medium' : defaultWeight;
...
const parseMarkdown = (markdown: string): MarkdownBlock[] => {
  ...
  while (index < lines.length) {
    ...
    if (line.toLowerCase().startsWith('/*script')) {
      ...
      blocks.push({ type: 'script', source: scriptLines.join('\n').trim() });
      index += 1;
      continue;
    }
      ...
      {blocks.map((block, index) => {
        if (block.type === 'script') {
          return (
            <ScriptRuntime
              ...
              isInDialog={isInDialog}
              renderMarkdown={(nestedMarkdown, nestedKey) => (
        ...
        if (block.type === 'quote') {
          return (
            <Column
              key={`quote-${index}`}
              className="border-border bg-background/30 gap-4 rounded-r-lg border-l-4 py-1 pl-4">
              {containsInputs ? (
                <InlineMarkdownWithInputs
                  text={block.text}
...
const MarkdownRenderer = (props: MarkdownRendererProps) => {
  const { playerOptions, roleOptions, scriptSources } = useContext(
    MarkdownRendererInputDataContext
  );
